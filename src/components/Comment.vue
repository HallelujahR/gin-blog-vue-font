  ...
  <div>
    <div v-for="comment in comments" :key="comment.id" class="comment">
      <div style="display:flex;align-items:center;gap:6px;">
        <span style="font-weight:600;color:#eaf1ff;font-size:15px;">{{ comment.author_name }}</span>
        <span style="font-size:12px;color:#93a4c8;">{{ formatDate(comment.created_at) }}</span>
      </div>
      <div style="margin:6px 0 8px 0;color:#cfdaf7;">{{ comment.content }}</div>
      <button style="font-size:13px;padding:2px 13px;border:1px solid #2a3242;background:#151d2b;" @click="replying = comment.id">回复</button>
      ...
      <div v-if="comment.children && comment.children.length" style="margin-left:21px;border-left:3px solid #2a3242;padding-left:14px;margin-top:11px;">
